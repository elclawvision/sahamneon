...
export default function LandingPage() {
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            setCountdown(prev => {
                ...
                let totalSeconds = parts[0] * 3600 + parts[1] * 60 + parts[2] - 1;
    ...
    const baseVisibleReviews = REVIEWS.slice(reviewPage * REVIEWS_PER_PAGE, (reviewPage + 1) * REVIEWS_PER_PAGE);
    
    const dbVisibleReviews = dbReviews.map(r => ({
        id: r.id,
        email: r.user_email.split('@')[0].slice(0, 3) + '***@' + r.user_email.split('@')[1],
        rating: r.rating,
    ...
    const finalVisibleReviews = [...dbVisibleReviews, ...baseVisibleReviews].slice(0, 10);
    ...
    return (
      ...
      `}</style>
                ...
                <div className={`fade-up ${reviewSection.inView ? 'visible' : ''}`}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
                        {[...dbReviews.map(r => ({
                            ...
                            text: r.comment,
                            verified: true
                        })), ...finalVisibleReviews].slice(0, 10).map(r => (
                    ...
                    <div style={{ textAlign: 'center', marginTop: '20px', padding: '16px', background: 'rgba(0,0,0,0.02)', borderRadius: '12px', border: '1px dashed var(--border)' }}>
                        ...
                        <button
                            ...
                            style={{ padding: '10px 24px', fontSize: '13px', width: 'auto' }}
                            onClick={() => navigate('/review')}
                        >
                            Tulis Review Anda Sekarang →
                        </button>
                    </div>
                </div>
            </section>
            ...
            <a
                href={`https://wa.me/62895325633487?text=${encodeURIComponent("Halo Admin Saham Ultimate, saya mau tanya...")}`}
                target="_blank"
